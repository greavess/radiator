...
extract_markers_metadata <- function(
  ...
  if (whitelist && blacklist) stop("Cannot have both a whitelist and blacklist")
...
check_coverage <- function(gds, genotypes.metadata.check = FALSE, stacks.haplo.check = FALSE, dart.check = FALSE){
  ...
  # This should be done when reading the stacks vcf
  # Want to avoid mutating the gds during operations that should only need read
  ...
  #   biallelic <- radiator::detect_biallelic_markers(data = gds)
  # }
  biallelic <- gdsfmt::read.gdsn(gdsfmt::index.gdsn(node = gds, path = "radiator/biallelic", silent = TRUE))
  is_stacks <- stringi::stri_detect_fixed(str = data.source, pattern = "Stacks")
  if (!biallelic && is_stacks) got.coverage <- NULL
...
generate_stats <- function(
  ...
  if (missing) {
    ...
    if (individuals) {
      # info
      i.missing <- SeqArray::seqMissing(
        gdsfile = gds,
        per.variant = FALSE,
        parallel = parallel.core
      )
      i.info$MISSING_PROP = round(i.missing, digits)
      # stats
      i.stats %<>%
        dplyr::bind_rows(
          tibble_stats(
